...
`Latency Offset` 슬라이더(`0 ~ 500ms`)로 소리가 화면보다 늦게 들릴 때 체감 지연을 맞출 수 있습니다.

![지연 보정 슬라이더와 안내 문구](docs/feature-latency.png)
...
BPM·박자·오프셋 값은 같은 브라우저에 저장되어, 다음에 열 때 이전 설정을 이어서 쓸 수 있습니다.
